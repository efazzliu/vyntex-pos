-- Per-terminal presence and management for the customer dashboard.
-- POS clients write only through the heartbeat RPC; venue owners read/rename through RLS.
...
drop policy if exists "pos_devices_owner_update" on public.pos_devices;

-- Existing string IDs become dashboard device rows immediately after migration.
